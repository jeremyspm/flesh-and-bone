...
const DECK = {
  bones:  { label:'Bones',   acc:'#f2b84b', ink:'#1b1303', items:BONES,   models:{ skeletal:'solid', sinuses:'solid' }, bind:['skeletal', 'sinuses'], noun:'bone', openLabel:'Glass skull' },
  ...
  senses: { label:'Eye & Ear', acc:'#ffc46b', ink:'#2a1a02', items:SENSES,  models:{ eye:'solid', ear:'solid', cochlea:'solid', retina:'solid' }, bind:['eye', 'ear', 'cochlea', 'retina'], noun:'part', home:'eye', view:[-72, 52], frame:{ pad:1.35, min:.05 }, schematic:'schematic · not to scale' },   // Module 3. Both BUILT: made-eye.js, made-ear.js
...
const MODELS = {
  ...
  walls:   { kind:'cell',   noun:'part',      note:'Building the tissues…', make:() => [...buildVessels(), ...buildHeartWall()] },
  uterwall:{ kind:'cell',   noun:'layer',     note:'Placing the organs…', make:buildUterusWall },
  ecg:     { kind:'cell',   noun:'part',      note:'Opening the chest…', make:buildECG },
  spiro:   { kind:'cell',   noun:'part',      note:'Filling the lungs…', make:buildSpirogram },
  glia:    { kind:'cell',   noun:'cell',      note:'Growing a neuron…', make:buildGlia },
  /* the cut-open figures (made-sections.js) and the two real sinus meshes of the source skull, which only show inside a glass skull */
  sinuses: { kind:'organ',  noun:'cavity',    note:'Unpacking the skeleton…' },
  longbone:{ kind:'cell',   noun:'part',      note:'Building the tissues…', make:buildLongBone },
  adrenalcut:{ kind:'cell', noun:'part',      note:'Placing the glands…', make:buildAdrenal },
...
async function loadModel(name, onProg) {
  ...
  root.traverse(o => {
    ...
    const info = { mesh:o, base, side, mat:ownerMat || matName, model:name, kind:soft ? 'tendon' : kind, colour:colour.clone(), clipX:clip ? clip.x : 0,
      ...
      openable:(kind === 'heart' && /^(left|right) (atrium|ventricle)$|^ascending aorta$|^pulmonary trunk$/.test(base)) || (kind === 'airway' && /lobe of/.test(base)) || (kind === 'bone' && SKULL_RE.test(base)),      // turns to glass when the question is about what is inside it        // a ventricle is a fluid space: drawn as glass, and a tap passes through it unless it is what was asked
      men:(kind === 'brain' && base === 'superior sagittal sinus') || (name === 'sinuses' && 'sin') };      // on stage with the meninges only
...
function addCaptions(root, model) {
  for (const C of CAPTIONS) { const R = MORE_REGIONS[C.region]; if (!R || R.model !== model) continue;
    ...
    const size = b.getSize(new THREE.Vector3()), h = Math.min(.022, Math.max(.009, Math.max(size.x, size.y) * .07)), lines = C.t.split('\n'), f = 46, pad = 14;
    ...
    x.font = `600 ${f}px system-ui, -apple-system, Segoe UI, sans-serif`; x.textBaseline = 'top'; x.textAlign = 'center'; lines.forEach((l, k) => { x.fillStyle = k ? 'rgba(255,255,255,.45)' : 'rgba(255,255,255,.7)'; x.fillText(l, cv.width / 2, pad + k * (f + 10)); });
    const tex = new THREE.CanvasTexture(cv); tex.colorSpace = THREE.SRGBColorSpace; const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map:tex, transparent:true, depthWrite:false, depthTest:false })), maxW = Math.max(size.x * .94, size.y * .45); let H = h * lines.length * 1.25; if (H * cv.width / cv.height > maxW) H = maxW * cv.height / cv.width;
...
function bindItems() {
  ...
  for (const it of Object.values(ITEM)) {
    ...
    it.infos = it.on ? pool.filter(i => i.base === it.on) : it.between ? pool.filter(i => it.between.includes(i.base)) : pool.filter(i => matchAny(it.m, i.base, i.mat));
    ...
    if (it.between) it.ok = it.infos.length === 2;
    if (!it.on && !it.between) it.infos.forEach(i => i.items.push(it));
...
function describe(info, pt) {
  const bw = pt && sectionOn && Object.values(ITEM).find(i => i.between && i.ok && i.deck === G.deck && i.infos.includes(info) && betweenHit(i, info, pt)); if (bw) return { title:bw.name, sub:'', item:bw, her:!!bw.her };
...
function paint(i) { const g = i.baseGhost || i.xr || i.peel, dimmed = (i.dimK || 1) < 1, m = i.mesh.material, was = m.transparent, glass = !!i.glassy || (i.soft && !(xray && xray.has(i) && !G.tr));
  m.transparent = !!g || glass || dimmed; m.opacity = i.baseGhost ? .11 : i.peel ? .13 : i.xr ? (i.kind === 'bone' ? .16 : .07) : glass ? (i.glassy || (i.openable ? (i.kind === 'bone' ? .13 : .3) : i.kind === 'heart' ? .1 : G.tr ? .5 : .42)) : dimmed ? .3 : 1; m.depthWrite = !(g || glass || dimmed); if (was !== m.transparent) m.needsUpdate = true; i.ghost = !!g; }
let xray = null;
...
let menOn = true;
function setMeninges(on) { if (on === 1) on = true; menOn = on; for (const i of REG) if (i.men) i.mesh.visible = on === true ? i.men === true : !!on && i.men === on; for (const o of menLines) o.visible = on === true; refreshPickables(); invalidate(); }
...
let sectionOn = false, sectionPlane = null, skullPlane = null;
const clipTo = (i, pl) => { if ((i.sect || null) === pl) return; const m = i.mesh.material; m.clippingPlanes = pl ? [pl] : null; m.side = pl || i.made ? THREE.DoubleSide : THREE.FrontSide; m.needsUpdate = true; i.sect = pl; };
function setSection(on) { if (on === 1) on = true; sectionOn = on; if (!skullPlane) skullPlane = new THREE.Plane(new THREE.Vector3(0, -1, 0), 1.645);
  for (const i of REG) if (i.model === 'skeletal' && SKULL_RE.test(i.base)) clipTo(i, on === 'skull' ? skullPlane : null);
  const hs = REG.filter(i => i.model === 'heart'); if (!hs.length) { refreshPickables(); invalidate(); return; } on = on === true;
  if (!sectionPlane) { const b = new THREE.Box3(); hs.filter(i => /^(left|right) (atrium|ventricle)$/.test(i.base)).forEach(i => b.union(i.box)); const n = new THREE.Vector3(.25, 0, -1).normalize(); sectionPlane = new THREE.Plane(n, -n.dot(b.getCenter(new THREE.Vector3())) + .004); }
  for (const i of hs) { if (/lobe of/.test(i.base)) { i.mesh.visible = !on; continue; }    clipTo(i, on ? sectionPlane : null); }
  refreshPickables(); invalidate(); }
/* a wall SHARED by two chambers (the septa): a tap is right where one chamber's wall lies within t of the other chamber */
const wverts = info => info.wv || (info.wv = (() => { const p = info.mesh.geometry.attributes.position, o = []; info.mesh.updateMatrixWorld(true); for (let k = 0; k < p.count; k += 2) o.push(new THREE.Vector3(p.getX(k), p.getY(k), p.getZ(k)).applyMatrix4(info.mesh.matrixWorld)); return o; })());
const nearTo = (pt, info) => { let d = Infinity; for (const q of wverts(info)) { const e = q.distanceToSquared(pt); if (e < d) d = e; } return Math.sqrt(d); };
const betweenHit = (it, info, pt) => { const other = it.infos.find(i => i !== info); return !!other && nearTo(pt, other) < it.t; };
let cutOn = false;
...
function regionFrame(name, az, elv) {
  ...
  if (!regionBox[name] || regionBox[name].isEmpty()) { const b = new THREE.Box3();      /* never trust a cached EMPTY box: a resize while the deck's model was still loading used to cache one, and the camera then never moved again */ REG.filter(i => i.model === (R.model || 'skeletal') && (!R.side || i.side === R.side || !i.side) && matchAny(R.m, i.base, i.mat)).forEach(i => b.union(i.box)); regionBox[name] = b; }
...
function cast(x, y) { const r = canvas.getBoundingClientRect(); ndc.set(((x - r.left) / r.width) * 2 - 1, -((y - r.top) / r.height) * 2 + 1); ray.setFromCamera(ndc, camera);
  const hits = ray.intersectObjects(pickables, false).filter(h => { const i = h.object.userData.info; return !(i.clipX && Math.abs(h.point.x) < i.clipX) && !(i.sect && i.sect.distanceToPoint(h.point) < 0) && !(i.soft && !(xray && xray.has(i)) && G.cur && !G.cur.it.infos.includes(i)); });
...
const G = { deck:S.o.deck, mode:S.o.mode, round:null, cur:null, tr:null, sex:'female', xHer:false, xPeel:false,
  ...
  async setDeck(deck) {
    ...
    setCut(false); setNerves(false); setMeninges(false); setOpen(false); setSection(false); applyDeck(); renderHome();
  },
  ...
  start(only) {
    ...
    this.xHer = this.xPeel = false; setCut(false); setNerves(this.mode === 'explore'); setMeninges(false); setOpen(false); setSection(false); applyDeck();
  ...
  next() {
    ...
    if (this.mode === 'find') {
      ...
      P.querySelector('.s').textContent = st === 'name' ? (it.sub ? `(${it.sub})` : '') : st === 'common' ? 'Tap it — what is its proper name?' : it.cut ? 'cut in half · seen from the left' : it.men ? (it.stage || 'schematic layers · thickness exaggerated') : it.section === 'skull' ? 'the skull cap is lifted off — you are looking down at the floor of the cranium, like her figure' : it.section ? 'the heart is cut open — a frontal section, like her figure' : it.open ? 'the chambers are shown as glass' : DECK[this.deck].schematic || '';
  ...
  anchor(it) { if (it.between) { if (!it.mid) { const [A, B] = it.infos, near = wverts(A).filter((q, k) => k % 3 === 0 && nearTo(q, B) < it.t), c = near.reduce((s, q) => s.add(q), new THREE.Vector3()).multiplyScalar(1 / Math.max(1, near.length)); it.mid = (near.reduce((a, q) => q.distanceToSquared(c) < a.distanceToSquared(c) ? q : a, near[0]) || A.box.getCenter(new THREE.Vector3())).clone(); } return it.mid.clone(); }
    if (!it.on && it.infos[0].anchor) return it.infos[0].anchor.clone();
  ...
  spot(it, hex, mode, dur) { if (it.on || it.between) { ring.show(this.anchor(it), mode === 'flash' ? 'good' : ''); } else glow(it.infos, hex, mode, dur); },
  ...
  finish() {
    ...
    $('#results').innerHTML = `<div class="rhead"><div class="big" style="--p:${pct}"><b>${pct}%</b></div><div><h2>${verdict}</h2><p>${R.right} of ${R.total} first time. ${tip}</p></div></div>
      ...
      <ul class="rlist">${R.log.map(l => `<li data-peek="${l.it.id}" class="${l.ok ? '' : 'miss'}"><i>${l.ok ? '✓' : '✕'}</i>${esc(l.it.name)}${l.it.sub ? ` <span style="color:var(--tx3);font-weight:500">(${esc(l.it.sub)})</span>` : ''}<small>${mast(l.it.id).b >= LOCK ? 'learned' : l.hinted && l.ok ? 'with a hint' : ''}</small></li>`).join('')}</ul>
  ...
  startTrace() {
    ...
    this.xHer = this.xPeel = false; if (T.sex) this.sex = T.sex; setCut(false); setNerves(false); setMeninges(!!T.men); setOpen(!!T.open); setSection(false); applyDeck();
  ...
  toggle(which) {
    ...
    if (which === 'sect') { setSection(sectionOn ? false : this.deck === 'bones' ? 'skull' : true); $('#xSect').setAttribute('aria-pressed', sectionOn); if (this.deck === 'bones') { if (sectionOn) { setOpen(false); $('#xOpen').setAttribute('aria-pressed', 'false'); } } else { const glass = sectionOn === true; setOpen(glass); $('#xOpen').setAttribute('aria-pressed', glass); } unglow(); callout.hide(); flyTo(this.deck === 'bones' ? regionFrame('head', 180, 78) : regionFrame('heart', 10, 2)); }
    if (which === 'open') { setOpen(!openOn); if (this.deck === 'bones') { setMeninges(openOn ? 'sin' : false); if (openOn) flyTo(regionFrame('head', 90, 5)); } $('#xOpen').setAttribute('aria-pressed', openOn); unglow(); callout.hide(); }
...
function renderHome() {
  const deckCard = d => { const her = DECK[d].items.filter(i => i.her && (i.ok || !DECK[d].bind.every(m => loaded[m]))), seen = her.some(i => S.m[i.id] && S.m[i.id].s), locked = her.filter(i => S.m[i.id] && S.m[i.id].b >= LOCK).length, p = her.length ? locked / her.length * 100 : 0;
    return `<button class="deck" data-deck="${d}" aria-pressed="${G.deck === d}"><span class="donut" style="--p:${p};--dc:${DECK[d].acc}"><span>${seen ? locked : '–'}</span></span><span><b>${DECK[d].label}</b><small>${seen ? `${locked} of ${her.length} learned` : `${her.length} on her list · not started`}</small></span></button>`; };
...
function goHome() { G.round = null; G.cur = null; G.tr = null; pins.clear(); unglow(); clearXray(); setPeel(null); setDim(null); ring.hide(); callout.hide(); toast(); setCut(false); setNerves(false); setMeninges(false); setOpen(false); setSection(false); applyDeck(); setState('home'); controls.autoRotate = true; renderHome(); }
...
$('#xHer').onclick = () => G.toggle('her'); $('#xPeel').onclick = () => G.toggle('peel'); $('#xCut').onclick = () => G.toggle('cut'); $('#xMen').onclick = () => G.toggle('men'); $('#xOpen').onclick = () => G.toggle('open'); $('#xSex').onclick = () => G.toggle('sex'); $('#xSect').onclick = () => G.toggle('sect');
...
window.FB = { S, G, REG, ITEM, DECK, TRACES, THREE, setCut, setMeninges, setOpen, setSection, setPeel, pins, setXray, clearXray, applyDeck, camera, controls, scene, groups, flyTo, frameBox, regionFrame, pointWorld, cast, toScreen, invalidate,
