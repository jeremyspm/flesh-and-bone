 ...
 *    figure listed here (21 Sep 2026). Her SLIDE figures are hosted by hs2-test3 (its content/hosted-figs.json: whole slides under img/slides/, pictures lifted from
 *    her decks at full resolution under img/figs/) — take one out there and the button here breaks, which tools/check-figrefs.mjs --live catches. `labelled:true` = her picture carries the names, so it must never show while a question is open. ── */
const M1 = 'https://jeremyspm.github.io/hs2-paper-m1/', M2 = 'https://jeremyspm.github.io/hs2-test2/', M3 = 'https://jeremyspm.github.io/hs2-test3/';
...
const EYE = [FG(M3, 'HS2DATA-9d23a8f4349d00eb.png', 'Module 3 formative · “Write the numbers that point to the parts of the eye”', 'A flat section with the cornea on the RIGHT and the optic nerve leaving at the lower left — the way ours opens. Hers is a line drawing with numbered boxes.'),
  FG(M3, 'HS2DATA-870f1dc664e81f99.jpg', 'Module 3 revision quiz · the eye, lettered a–s', 'A 3D cut-away, cornea on the right. The thin line running from the back of the lens to the optic disc is the hyaloid canal; the vessels fan out from the disc.')];
const EAR = [FG(M3, 'HS2DATA-34302bd5d442c8db.png', 'Module 3 formative · the ear (three questions use it)', 'The same left-to-right layout as ours: pinna, canal, eardrum (hers is green), the three ossicles, then the semicircular canals and the cochlea (purple), with the Eustachian tube running down and away. Hers has no labels.')];
const COCHLEA = [FG(M3, 'HS2DATA-5af5aaa0f83509e8.png', 'Module 3 formative · transduction of sound in the cochlea', 'One turn of the cochlea cut across, as ours is: scala vestibuli on top, scala media in the middle with the organ of Corti sitting on the basilar membrane under the tectorial membrane, scala tympani below.', 1),
  FG(M3, 'HS2DATA-efb1926abee633bb.webp', 'Module 3 revision quiz · organ of Corti', 'The same cross-section again from a second source — same three chambers in the same order, with the nerve fibres leaving toward the centre of the cochlea.', 1)];
/* her slides (Module 3 revision deck, her Light & Seeing and Repro Female lectures) */
const SL = 'Her Module 3 revision slides · ';
const OVCYCLE = FG(M3, 'figs/rev3-s2-ovarian-cycle.jpg', SL + 'slide 2, the ovarian cycle', 'Her ovary cut open with the stages round the edge in order — primary, secondary, vesicular follicle, ovulation, corpus luteum forms, regression, corpus albicans — running clockwise from the top, as ours does. A small uterus above shows where the ovary sits; the vessels come in from the left.', 1);
const UTART = FG(M3, 'figs/rev3-s2-uterine-arteries.png', SL + 'slide 2, arteries of the uterus wall', 'The layout ours copies: side-on, outside on the LEFT, the cavity on the right. Uterine artery → arcuate → radial through the myometrium, then basal (straight) arteries and coiled spiral arteries in the endometrium. Hers draws no perimetrium and does not split the endometrium into its two layers.', 1);
const UTSUP = FG(M3, 'figs/repro-female-s19-uterine-supply.jpg', 'Her Repro Female lecture · slide 19, uterine vascular supply', 'The same wall FLIPPED: outside (perimetrium, uterine artery) on the RIGHT, the cavity on the left, with the functional and basal layers marked along the top and the spiral arteries in the functional layer. Her slide text: spiral arteries → stratum functionalis, straight arteries → stratum basalis.', 1);
const EYESL = FG(M3, 'slides/2023-Revision-mod-3-22-slide2-15.jpg', SL + 'slide 15, the eye', 'A labelled side section with the cornea on the LEFT — the other way round from ours and her quiz figures. Her notes beside it: fovea = only cones; optic disc = no rods or cones, the blind spot.', 1);
const EARSL = FG(M3, 'slides/2023-Revision-mod-3-22-slide2-19.jpg', SL + 'slide 19, external and middle ear', 'Numbered 1–5 along the way sound goes: ear canal, tympanic membrane, the three ossicles, oval window, cochlea — outside on the left, as ours. A small second drawing shows the Eustachian tube running down from the middle ear.', 1);
const INNERSL = FG(M3, 'slides/2023-Revision-mod-3-22-slide2-20.jpg', SL + 'slide 20, the inner ear', 'Perilymph drawn YELLOW, endolymph BLUE; semicircular ducts, utricle, saccule and cochlea labelled. On the right, the cochlea uncoiled: in at the oval window, along the scala vestibuli, back by the scala tympani to the round window.', 1);
const CORTISL = FG(M3, 'slides/2023-Revision-mod-3-22-slide2-21.jpg', SL + 'slide 21, the organ of Corti', 'A close-up of the organ of Corti only: inner and outer hair cells under the tectorial membrane, sitting on the basilar membrane. The small drawing at the top right is the whole turn, showing where the close-up sits. Ours is the whole turn.', 1);
const RETINA = [FG(M3, 'slides/2026-LIGHT-AND-SEEING-10.jpg', 'Her Light & Seeing lecture · slide 10, the eye', 'Not the layers: a whole-eye section with the retina drawn as a dashed line — green where the rods are (round the edges), red where the cones crowd (the fovea). Ours zooms into one small patch of that line.', 1),
  FG(M3, 'slides/2026-LIGHT-AND-SEEING-11.jpg', 'Her Light & Seeing lecture · slide 11, rods and cones', 'Just the two receptor cells side by side, a rod and a cone, with her notes on each. She draws no bipolar or ganglion cells; the layer order ours shows is from the words of her revision slide 16, not from a picture.', 1)];
export const FIGREF = {
  eye:[...EYE, EYESL], eyeFront:[...EYE, EYESL], ear:[...EAR, EARSL], earMid:[...EAR, EARSL], earIn:[...EAR, INNERSL], seCochlea:[...COCHLEA, CORTISL],
  seRetina:RETINA, uterwall:[UTART, UTSUP],
  rpOvary:[FG(M3, 'HS2DATA-e3645ba42e2642d3.png', 'Fertility cases quiz · the “ovarian cycle” figure', 'The same convention as ours — stages in order round the edge, the Graafian follicle at the surface, the egg leaving beside it, then corpus luteum and corpus albicans — but hers runs ANTICLOCKWISE with the vessels entering on the right. It labels the cells round the released egg corona radiata; her drop-down key calls that layer zona radiata.', 1), OVCYCLE],
  rpTubule:[FG(M3, 'HS2DATA-d37cc11e291479fb.jpg', 'Module 3 revision quiz · spermatogenesis', 'On the right, the round cross-section ours copies (lumen in the middle, Leydig cells and a capillary outside) as a microscope photo. On the left, her flow chart — which runs from the spermatogonium at the TOP down to the sperm at the bottom.', 1)],
  rpSperm:[FG(M3, 'HS2DATA-d84b6f38368cc300.png', 'Reproduction MC quiz · normal and abnormal sperm', 'Hers labels a normal sperm simply head, middle section and tail, beside drawings of abnormal heads, necks and tails. Ours adds what is inside: acrosome, chromatin, mitochondria.', 1)],
  rpFemale:[FG(M3, 'HS2DATA-1c9309897b1c6922.jpg', 'Formative: female reproduction · mix and match 1–8', 'A front view of the uterus, tubes and ovaries inside a body outline, with eight numbered pointers. Ours is the same view in 3D.')],
  rpMale:[FG(M3, 'HS2DATA-73c637315e48328d.png', 'Module 3 revision quiz · the male system', 'A side-on section with the front of the body on the LEFT and numbered pointers — the way ours opens.')],
