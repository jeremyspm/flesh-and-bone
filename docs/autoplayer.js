...
window.fabAutoplay = async (deck = 'bones', mode = 'find') => {
  ...
  const findPt = it => { if (it.between) {                          // a wall shared by two chambers (the septa): aim at a vertex of one chamber that lies within t of the other
      const [A, B] = it.infos, bv = [], q = B.mesh.geometry.attributes.position, p = A.mesh.geometry.attributes.position; for (let k = 0; k < q.count; k += 3) bv.push(new THREE.Vector3().fromBufferAttribute(q, k).applyMatrix4(B.mesh.matrixWorld));
      for (let k = 0; k < p.count; k += 3) { v.fromBufferAttribute(p, k).applyMatrix4(A.mesh.matrixWorld); if (!bv.some(b => b.distanceTo(v) < it.t * .7)) continue; const s = FB.toScreen(v), e = document.elementFromPoint(s.x, s.y); if (!e || e.id !== 'c') continue;
        const h = FB.cast(s.x, s.y); if (h && h.object.userData.info === A && h.point.distanceTo(v) < .004) return s; } return null; }
    if (it.on && !it.zone) {                       // a point landmark (the carina, the head of the femur): aim INSIDE its sphere, not anywhere on the bone
